...
def _excursoes_posicao(
    ...
    return {
        "maximum_favorable_excursion": max(
            0.0,
            float(max(valores_maximos) / base - 1.0),
        ),
        "maximum_adverse_excursion": min(
            0.0,
            float(min(valores_minimos) / base - 1.0),
        ),
    }
...
def enriquecer_diagnosticos_operacoes(
    ...
    """Acrescenta excursões, contrafactuais e custo de oportunidade às operações."""
    ...
    for indice, compra in enumerate(linhas):
        ...
        ativo_destino = str(
            compra.get("rotation_to_asset") or compra.get("asset") or ""
        ).strip()
        ...
        retorno_escolhido = _retorno_abertura_abertura(
            quadros,
            ativo_destino,
            inicio_em,
            fim_em,
        )
        retorno_anterior = _retorno_abertura_abertura(
            quadros,
            ativo_origem,
            inicio_em,
            fim_em,
        ...
        for simbolo in universo:
            valor = _retorno_abertura_abertura(
                quadros,
                simbolo,
                inicio_em,
                fim_em,
        ...
        if retornos_alternativos:
            melhor_ativo, melhor_retorno = max(
                retornos_alternativos,
        ...
        compra.update(
            {
                "subsequent_position_return": _numero(
                    linha_saida.get("position_return")
                ),
                ...
                "rotation_regret": (
                    max(0.0, -valor_adicionado)
                    if valor_adicionado is not None
                    else None
                ),
                ...
                "maximum_favorable_excursion": _numero(
                    linha_saida.get("maximum_favorable_excursion")
                ),
                "maximum_adverse_excursion": _numero(
                    linha_saida.get("maximum_adverse_excursion")
                ),
                "profit_capture_ratio": _numero(
                    linha_saida.get("profit_capture_ratio")
                ),
                "subsequent_holding_days": _numero(
                    linha_saida.get("holding_bars")
                ),
            }
        )
    return linhas
