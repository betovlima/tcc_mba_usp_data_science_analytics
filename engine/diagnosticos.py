...
def _normalizar_timestamp(value: Any) -> pd.Timestamp | None:
    if value is None:
        return None
    try:
...
def _normalizar_numero(value: Any) -> float | None:
    try:
        number = float(value)
    except (TypeError, ValueError):
        return None
...
def _quadro_ativo(frames: dict[str, pd.DataFrame], symbol: str) -> pd.DataFrame | None:
...
def _retorno_abertura_a_abertura(
    frames: dict[str, pd.DataFrame],
    symbol: str,
    start_at: Any,
    end_at: Any,
) -> float | None:
    frame = _quadro_ativo(frames, symbol)
    start = _normalizar_timestamp(start_at)
    end = _normalizar_timestamp(end_at)
    if frame is None or start is None or end is None or end <= start:
        return None
    try:
        start_open = _normalizar_numero(frame.loc[start, "open"])
        end_open = _normalizar_numero(frame.loc[end, "open"])
...
def _excursoes_posicao(
    ...
    frame = _quadro_ativo(frames, symbol)
    start = _normalizar_timestamp(entry_at)
    end = _normalizar_timestamp(exit_at)
    base = _normalizar_numero(entry_price)
    ...
    exit_open = None
    try:
        exit_open = _normalizar_numero(frame.loc[end, "open"])
    except (KeyError, TypeError):
        pass
...
def enriquecer_diagnosticos_operacoes(
    ...
    for row in rows:
        ...
        realized_return = _normalizar_numero(row.get("position_return"))
        mfe = _normalizar_numero(excursions.get("maximum_favorable_excursion"))
    ...
    for index, buy in enumerate(rows):
        ...
        exit_row = _proxima_saida(rows, index, to_asset)
        if exit_row is None:
            continue
        start_at = buy.get("timestamp")
        end_at = exit_row.get("timestamp")
        chosen_return = _retorno_abertura_a_abertura(frames, to_asset, start_at, end_at)
        previous_return = _retorno_abertura_a_abertura(frames, from_asset, start_at, end_at)

        alternative_returns: list[tuple[str, float]] = []
        for symbol in universe:
            value = _retorno_abertura_a_abertura(frames, symbol, start_at, end_at)
        ...
        buy.update(
            {
                "subsequent_position_return": _normalizar_numero(exit_row.get("position_return")),
                ...
                "maximum_favorable_excursion": _normalizar_numero(exit_row.get("maximum_favorable_excursion")),
                "maximum_adverse_excursion": _normalizar_numero(exit_row.get("maximum_adverse_excursion")),
                "profit_capture_ratio": _normalizar_numero(exit_row.get("profit_capture_ratio")),
                "subsequent_holding_days": _normalizar_numero(exit_row.get("holding_bars")),
            }
        )
